...
void InternalAdbcSetError(struct AdbcError *error, const char *format, ...) ADBC_CHECK_PRINTF_ATTRIBUTE;
...
int ADBC_CHECK_PRINTF_ATTRIBUTE InternalAdbcStringBuilderAppend(struct InternalAdbcStringBuilder *builder,
                                                                const char *fmt,
                                                                ...);
...
#define RAISE(CODE, EXPR, ERRMSG, ERROR)                                                                               \
    do {                                                                                                               \
        if (!(EXPR)) {                                                                                                 \
            InternalAdbcSetError(ERROR, "%s failed: %s\nDetail: %s:%d", #EXPR, ERRMSG, __FILE__, __LINE__);            \
            return ADBC_STATUS_##CODE;                                                                                 \
        }                                                                                                              \
    } while (0)
