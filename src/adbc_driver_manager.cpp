...
class RegistryKey {
public:
    RegistryKey(HKEY root, const std::wstring_view subkey) noexcept : root_(root), key_(nullptr) {
    ...
    std::wstring GetString(const std::wstring &name, std::wstring default_value) {
        ...
        result = RegQueryValueExW(key_, name.data(), nullptr, &type, reinterpret_cast<LPBYTE>(value.data()), &size);
        if (result != ERROR_SUCCESS) return default_value;
        return value;
    }
    ...
    int32_t GetInt(const std::wstring &name, const int32_t default_value) {
        ...
        DWORD valueType;
        auto result = RegQueryValueExW(key_, name.data(), nullptr, &valueType, (LPBYTE)&dwValue, &dataSize);
        if (result != ERROR_SUCCESS) return default_value;
...
AdbcStatusCode LoadDriverFromRegistry(HKEY root,
    ...
    if (info.manifest_version != 1) {
        SetError(error,
                 "Driver manifest version '" + std::to_string(info.manifest_version) +
...
AdbcStatusCode LoadDriverManifest(const std::filesystem::path &driver_manifest,
    ...
    if (entrypoint) {
        if (auto *ep = entrypoint.as_string()) {
            ...
            SetError(error, "Driver entrypoint not a string in manifest '"s + driver_manifest.string() + "'"s);
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
    }
    ...
    if (toml::table *platforms = driver.as_table()) {
        ...
        info.lib_path = path->get();
        if (info.lib_path.empty()) {
            SetError(error, "Driver path is an empty string in manifest '"s + driver_manifest.string() + "'"s);
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
        return ADBC_STATUS_OK;
    }
    SetError(error,
             "Driver path not defined in manifest '"s + driver_manifest.string() +
...
bool HasExtension(const std::filesystem::path &path, const std::string &ext) {
    ...
    return path_ext.size() == wext.size() && _wcsnicmp(path_ext.data(), wext.data(), wext.size()) == 0;
#else
...
struct ManagedLibrary {
    ...
    AdbcStatusCode GetDriverInfo(const std::string_view driver_name,
        ...
        if (driver_path.has_extension()) {
            ...
            SetError(error, "Driver name has unrecognized extension: " + driver_path.extension().string());
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
    ...
    AdbcStatusCode SearchPathsForDriver(const std::filesystem::path &driver_path,
        ...
        for (const auto &[source, search_path] : search_paths) {
            if (source == SearchPathSource::kRegistry || source == SearchPathSource::kUnset ||
                source == SearchPathSource::kDoesNotExist || source == SearchPathSource::kDisabledAtCompileTime ||
                source == SearchPathSource::kDisabledAtRunTime || source == SearchPathSource::kOtherError) {
                continue;
            }
            ...
            if (std::filesystem::exists(full_path)) {
                ...
                if (status == ADBC_STATUS_OK) {
                    ...
                    search_paths.insert(search_paths.end(), extra_debug_info.begin(), extra_debug_info.end());
                    return status;
    ...
    AdbcStatusCode FindDriver(const std::filesystem::path &driver_path,
            ...
            if (load_options & ADBC_LOAD_FLAG_SEARCH_ENV) {
                ...
                if (!venv.empty()) {
                    for (const auto &[_, venv_path] : venv) {
                        search_paths.emplace_back(SearchPathSource::kConda, venv_path / "etc" / "adbc" / "drivers");
                    }
                }
            } else {
                search_paths.emplace_back(SearchPathSource::kDisabledAtRunTime,
        ...
        if (load_options & ADBC_LOAD_FLAG_SEARCH_USER) {
            ...
            auto status = LoadDriverFromRegistry(HKEY_CURRENT_USER, driver_path.native(), info, error);
            if (status == ADBC_STATUS_OK) {
                return Load(info.lib_path.native(), {}, error);
            }
            if (error && error->message) {
                ...
                search_paths.emplace_back(SearchPathSource::kRegistry, "not found in HKEY_CURRENT_USER");
            }
        ...
        if (load_options & ADBC_LOAD_FLAG_SEARCH_SYSTEM) {
            ...
            auto status = LoadDriverFromRegistry(HKEY_LOCAL_MACHINE, driver_path.native(), info, error);
            if (status == ADBC_STATUS_OK) {
                return Load(info.lib_path.native(), {}, error);
            }
            if (error && error->message) {
                ...
                search_paths.emplace_back(SearchPathSource::kRegistry, "not found in HKEY_LOCAL_MACHINE");
            }
        ...
        SearchPaths more_search_paths = GetSearchPaths(load_options & ~ADBC_LOAD_FLAG_SEARCH_ENV);
        ...
        if (status == ADBC_STATUS_NOT_FOUND) {
            if (!(load_options & ADBC_LOAD_FLAG_SEARCH_USER)) {
                ...
                more_search_paths.emplace_back(SearchPathSource::kDisabledAtRunTime, std::move(message));
            }
            ...
            search_paths.insert(search_paths.end(), more_search_paths.begin(), more_search_paths.end());
            info.lib_path = driver_path;
            return Load(driver_path.native(), search_paths, error);
        }
        return status;
    ...
    AdbcStatusCode Load(const string_type &library, const SearchPaths &attempted_paths, struct AdbcError *error) {
...
AdbcStatusCode DatabaseSetOption(struct AdbcDatabase *database,
                                 const char *key,
                                 const char *value,
                                 struct AdbcError *error) {
...
AdbcStatusCode ConnectionGetObjects(struct AdbcConnection *,
                                    int,
                                    const char *,
                                    const char *,
                                    const char *,
...
AdbcStatusCode ConnectionGetStatistics(struct AdbcConnection *,
                                       const char *,
                                       const char *,
                                       const char *,
                                       char,
...
AdbcStatusCode ConnectionGetTableSchema(struct AdbcConnection *,
                                        const char *,
                                        const char *,
                                        const char *,
                                        struct ArrowSchema *,
...
AdbcStatusCode ConnectionSetOptionBytes(struct AdbcConnection *,
                                        const char *,
                                        const uint8_t *,
                                        size_t,
                                        struct AdbcError *error) {
...
AdbcStatusCode StatementBind(struct AdbcStatement *,
                             struct ArrowArray *,
                             struct ArrowSchema *,
                             struct AdbcError *error) {
...
AdbcStatusCode StatementBindStream(struct AdbcStatement *, struct ArrowArrayStream *, struct AdbcError *error) {
...
AdbcStatusCode StatementSetOptionBytes(struct AdbcStatement *,
                                       const char *,
                                       const uint8_t *,
                                       size_t,
                                       struct AdbcError *error) {
...
std::vector<std::filesystem::path> InternalAdbcParsePath(const std::string_view path) {
    ...
    // remove empty paths
    result.erase(std::remove_if(result.begin(), result.end(), [](const auto &p) { return p.empty(); }), result.end());
    return result;
}
...
AdbcStatusCode AdbcDatabaseInit(struct AdbcDatabase *database, struct AdbcError *error) {
    ...
    if (!args->init_func) {
        ...
        if (args->driver.empty()) {
            SetError(error, "Must provide 'driver' parameter (or encode driver in 'uri' parameter)");
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
    }
    ...
    if (args->init_func) {
        ...
        status = AdbcFindLoadDriver(args->driver.c_str(),
    ...
    for (const auto &option : bytes_options) {
        status =
            database->private_driver->DatabaseSetOptionBytes(database,
                                                             option.first.c_str(),
    ...
    for (const auto &option : int_options) {
        status = database->private_driver->DatabaseSetOptionInt(database,
                                                                option.first.c_str(),
                                                                option.second,
                                                                &driver_error.error);
        if (status != ADBC_STATUS_OK) break;
    ...
    for (const auto &option : double_options) {
        status = database->private_driver->DatabaseSetOptionDouble(database,
                                                                   option.first.c_str(),
                                                                   option.second,
                                                                   &driver_error.error);
        if (status != ADBC_STATUS_OK) break;
...
AdbcStatusCode AdbcConnectionGetObjects(struct AdbcConnection *connection,
    ...
    INIT_ERROR(error, connection);
    WRAP_STREAM(connection->private_driver->ConnectionGetObjects(connection,
                                                                 depth,
                                                                 catalog,
                                                                 db_schema,
                                                                 table_name,
...
AdbcStatusCode AdbcConnectionGetStatistics(struct AdbcConnection *connection,
    ...
    INIT_ERROR(error, connection);
    WRAP_STREAM(connection->private_driver
                    ->ConnectionGetStatistics(connection, catalog, db_schema, table_name, approximate == 1, out, error),
                out,
                connection);
}
...
AdbcStatusCode AdbcConnectionGetTableSchema(struct AdbcConnection *connection,
    ...
    INIT_ERROR(error, connection);
    return connection->private_driver
        ->ConnectionGetTableSchema(connection, catalog, db_schema, table_name, schema, error);
}
...
AdbcStatusCode AdbcConnectionInit(struct AdbcConnection *connection,
    ...
    for (const auto &option : options) {
        status = database->private_driver->ConnectionSetOption(connection,
                                                               option.first.c_str(),
                                                               option.second.c_str(),
                                                               error);
        if (status != ADBC_STATUS_OK) return status;
    ...
    for (const auto &option : bytes_options) {
        status =
            database->private_driver->ConnectionSetOptionBytes(connection,
                                                               option.first.c_str(),
                                                               reinterpret_cast<const uint8_t *>(option.second.data()),
                                                               option.second.size(),
                                                               error);
...
AdbcStatusCode AdbcConnectionReadPartition(struct AdbcConnection *connection,
    ...
    INIT_ERROR(error, connection);
    WRAP_STREAM(connection->private_driver
                    ->ConnectionReadPartition(connection, serialized_partition, serialized_length, out, error),
                out,
                connection);
}
...
AdbcStatusCode AdbcFindLoadDriver(const char *driver_name,
    ...
    AdbcStatusCode status = library.GetDriverInfo(driver_name, load_options, additional_paths, info, error);
    if (status != ADBC_STATUS_OK) {
...
AdbcStatusCode AdbcLoadDriver(const char *driver_name,
    ...
    return AdbcFindLoadDriver(driver_name,
                              entrypoint,
                              version,
