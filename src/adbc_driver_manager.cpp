...
#define NTDDI_VERSION                                                          \
  0x0A00000C // For SHGetKnownFolderPath in ShlObj_core.h in ShlObj.h
#endif
...
InternalAdbcParsePath(const std::string_view path);
ADBC_EXPORT
...
using SearchPaths =
    std::vector<std::pair<SearchPathSource, std::filesystem::path>>;

void AddSearchPathsToError(const SearchPaths &search_paths,
...
std::string CheckNonPrintableLibraryName(const std::string &name) {
  ...
  std::string error_message =
      "Note: driver name may have non-printable characters: `";
...
public:
  ...
  std::wstring GetString(const std::wstring &name, std::wstring default_value) {
    ...
    DWORD size = 0;
    auto result =
        RegQueryValueExW(key_, name.data(), nullptr, &type, nullptr, &size);
...
AdbcStatusCode LoadDriverFromRegistry(HKEY root,
  ...
  if (!dkey.is_open()) {
    std::string error_message =
        "SOFTWARE\\ADBC\\DRIVERS has no entry for driver \""s;
  ...
  if (info.manifest_version != 1) {
    SetError(error, "Driver manifest version '" +
                        std::to_string(info.manifest_version) +
...
SearchPaths GetEnvPaths(const char_type *env_var) {
  ...
  DWORD actual_size =
      GetEnvironmentVariableW(env_var, path_var.data(), required_size);
...
struct ManagedLibrary {
  ...
  AdbcStatusCode GetDriverInfo(
    ...
    const bool allow_relative_paths =
        load_options & ADBC_LOAD_FLAG_ALLOW_RELATIVE_PATHS;
    ...
    if (driver_path.is_absolute()) {
      // if we have an absolute path without an extension, first see if there's
    ...
    return FindDriver(driver_path, load_options, additional_search_paths, info,
                      error);
  }
  ...
  AdbcStatusCode SearchPathsForDriver(const std::filesystem::path &driver_path,
    ...
    for (const auto &[source, search_path] : search_paths) {
      if (source == SearchPathSource::kRegistry ||
          source == SearchPathSource::kUnset ||
      ...
      if (std::filesystem::exists(full_path)) {
        ...
        auto status =
            LoadDriverManifest(full_path, info, &intermediate_error.error);
        if (status == ADBC_STATUS_OK) {
  ...
  AdbcStatusCode
  FindDriver(const std::filesystem::path &driver_path,
    ...
    SearchPaths search_paths;
    {
      ...
      auto status =
          SearchPathsForDriver(driver_path, search_paths, info, error);
    ...
    if (load_options & ADBC_LOAD_FLAG_SEARCH_USER) {
      ...
      auto status = LoadDriverFromRegistry(HKEY_CURRENT_USER,
                                           driver_path.native(), info, error);
      if (status == ADBC_STATUS_OK) {
        return Load(info.lib_path.native(), {}, error);
      }
    ...
    if (load_options & ADBC_LOAD_FLAG_SEARCH_SYSTEM) {
      ...
      auto status = LoadDriverFromRegistry(HKEY_LOCAL_MACHINE,
                                           driver_path.native(), info, error);
      if (status == ADBC_STATUS_OK) {
        return Load(info.lib_path.native(), {}, error);
      }
    ...
    auto status =
        SearchPathsForDriver(driver_path, more_search_paths, info, error);
    if (status == ADBC_STATUS_NOT_FOUND) {
      if (!(load_options & ADBC_LOAD_FLAG_SEARCH_USER)) {
        std::filesystem::path user_config_dir = InternalAdbcUserConfigDir();
  ...
  AdbcStatusCode Load(const string_type &library,
  ...
  AdbcStatusCode Lookup(const char *name, void **func,
                        struct AdbcError *error) {
#if defined(_WIN32)
...
struct ManagerDriverState {
  ...
  AdbcStatusCode (*driver_release)(struct AdbcDriver *driver,
                                   struct AdbcError *error);
...
static AdbcStatusCode ReleaseDriver(struct AdbcDriver *driver,
                                    struct AdbcError *error) {
...
void ErrorArrayStreamRelease(struct ArrowArrayStream *stream) {
  ...
  auto *private_data =
      reinterpret_cast<struct ErrorArrayStream *>(stream->private_data);
...
int ErrorArrayStreamGetNext(struct ArrowArrayStream *stream,
...
struct AdbcErrorDetail ErrorGetDetail(const struct AdbcError *error,
                                      int index) {
...
AdbcStatusCode DatabaseGetOptionBytes(struct AdbcDatabase *database,
...
AdbcStatusCode DatabaseGetOptionInt(struct AdbcDatabase *database,
...
AdbcStatusCode DatabaseGetOptionDouble(struct AdbcDatabase *database,
...
AdbcStatusCode DatabaseSetOptionBytes(struct AdbcDatabase *database,
...
AdbcStatusCode DatabaseSetOptionInt(struct AdbcDatabase *database,
                                    const char *key, int64_t value,
                                    struct AdbcError *error) {
...
AdbcStatusCode DatabaseSetOptionDouble(struct AdbcDatabase *database,
                                       const char *key, double value,
                                       struct AdbcError *error) {
...
AdbcStatusCode ConnectionCommit(struct AdbcConnection *,
                                struct AdbcError *error) {
...
AdbcStatusCode ConnectionGetOption(struct AdbcConnection *connection,
...
AdbcStatusCode ConnectionGetOptionInt(struct AdbcConnection *connection,
                                      const char *key, int64_t *value,
                                      struct AdbcError *error) {
...
AdbcStatusCode ConnectionRollback(struct AdbcConnection *,
                                  struct AdbcError *error) {
...
AdbcStatusCode ConnectionSetOption(struct AdbcConnection *, const char *,
                                   const char *, struct AdbcError *error) {
...
AdbcStatusCode ConnectionSetOptionInt(struct AdbcConnection *connection,
                                      const char *key, int64_t value,
                                      struct AdbcError *error) {
...
AdbcStatusCode StatementBindStream(struct AdbcStatement *,
                                   struct ArrowArrayStream *,
                                   struct AdbcError *error) {
...
AdbcStatusCode StatementCancel(struct AdbcStatement *statement,
                               struct AdbcError *error) {
...
AdbcStatusCode StatementGetOption(struct AdbcStatement *statement,
                                  const char *key, char *value, size_t *length,
                                  struct AdbcError *error) {
...
AdbcStatusCode StatementGetOptionBytes(struct AdbcStatement *statement,
                                       const char *key, uint8_t *value,
                                       size_t *length,
                                       struct AdbcError *error) {
...
AdbcStatusCode StatementGetOptionInt(struct AdbcStatement *statement,
                                     const char *key, int64_t *value,
                                     struct AdbcError *error) {
...
AdbcStatusCode StatementGetOptionDouble(struct AdbcStatement *statement,
                                        const char *key, double *value,
                                        struct AdbcError *error) {
...
AdbcStatusCode StatementPrepare(struct AdbcStatement *,
                                struct AdbcError *error) {
...
AdbcStatusCode StatementSetOptionInt(struct AdbcStatement *statement,
                                     const char *key, int64_t value,
                                     struct AdbcError *error) {
...
AdbcStatusCode StatementSetOptionDouble(struct AdbcStatement *statement,
                                        const char *key, double value,
                                        struct AdbcError *error) {
...
AdbcStatusCode StatementSetSubstraitPlan(struct AdbcStatement *,
                                         const uint8_t *, size_t,
                                         struct AdbcError *error) {
...
int AdbcErrorGetDetailCount(const struct AdbcError *error) {
  if (error->vendor_code == ADBC_ERROR_VENDOR_CODE_PRIVATE_DATA &&
      error->private_data && error->private_driver &&
...
const struct AdbcError *
AdbcErrorFromArrayStream(struct ArrowArrayStream *stream,
                         AdbcStatusCode *status) {
  ...
  auto *private_data =
      reinterpret_cast<struct ErrorArrayStream *>(stream->private_data);
  auto *error = private_data->private_driver->ErrorFromArrayStream(
      &private_data->stream, status);
  if (error) {
...
AdbcStatusCode AdbcDatabaseGetOption(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    ...
    return database->private_driver->DatabaseGetOption(database, key, value,
                                                       length, error);
  }
  const auto *args =
      reinterpret_cast<const TempDatabase *>(database->private_data);
...
AdbcStatusCode AdbcDatabaseGetOptionBytes(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseGetOptionBytes(
        database, key, value, length, error);
  }
  const auto *args =
      reinterpret_cast<const TempDatabase *>(database->private_data);
...
AdbcStatusCode AdbcDatabaseGetOptionInt(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseGetOptionInt(database, key, value,
                                                          error);
  }
  const auto *args =
      reinterpret_cast<const TempDatabase *>(database->private_data);
...
AdbcStatusCode AdbcDatabaseGetOptionDouble(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseGetOptionDouble(database, key,
                                                             value, error);
  }
  const auto *args =
      reinterpret_cast<const TempDatabase *>(database->private_data);
...
AdbcStatusCode AdbcDatabaseSetOptionBytes(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseSetOptionBytes(
        database, key, value, length, error);
  }

  TempDatabase *args = reinterpret_cast<TempDatabase *>(database->private_data);
  args->bytes_options[key] =
...
AdbcStatusCode AdbcDatabaseSetOptionInt(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseSetOptionInt(database, key, value,
                                                          error);
  }
...
AdbcStatusCode AdbcDatabaseSetOptionDouble(struct AdbcDatabase *database,
  ...
  if (database->private_driver) {
    INIT_ERROR(error, database);
    return database->private_driver->DatabaseSetOptionDouble(database, key,
                                                             value, error);
  }
...
AdbcStatusCode
AdbcDriverManagerDatabaseSetLoadFlags(struct AdbcDatabase *database,
                                      AdbcLoadFlags flags,
                                      struct AdbcError *error) {
  if (database->private_driver) {
...
AdbcStatusCode AdbcDriverManagerDatabaseSetAdditionalSearchPathList(
    struct AdbcDatabase *database, const char *path_list,
    struct AdbcError *error) {
  if (database->private_driver) {
    SetError(error,
             "Cannot SetAdditionalSearchPathList after AdbcDatabaseInit");
    return ADBC_STATUS_INVALID_STATE;
  }
...
AdbcStatusCode
AdbcDriverManagerDatabaseSetInitFunc(struct AdbcDatabase *database,
...
AdbcStatusCode AdbcDatabaseInit(struct AdbcDatabase *database,
  ...
  if (!args->init_func) {
    ...
    if (args->driver.empty()) {
      SetError(error, "Must provide 'driver' parameter (or encode driver in "
                      "'uri' parameter)");
      return ADBC_STATUS_INVALID_ARGUMENT;
    }
  }
  ...
  for (const auto &option : bytes_options) {
    status = database->private_driver->DatabaseSetOptionBytes(
        ...
        option.second.size(), &driver_error.error);
    if (status != ADBC_STATUS_OK)
      break;
  }
...
AdbcStatusCode AdbcConnectionGetObjects(
  ...
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionGetObjects: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetObjects(
                  connection, depth, catalog, db_schema, table_name,
...
AdbcStatusCode AdbcConnectionGetOption(struct AdbcConnection *connection,
  ...
  if (!connection->private_data) {
    SetError(error,
             "AdbcConnectionGetOption: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  if (!connection->private_driver) {
    // Init not yet called, get the saved option
    const auto *args =
        reinterpret_cast<const TempConnection *>(connection->private_data);
    const auto it = args->options.find(key);
    if (it == args->options.end()) {
      return ADBC_STATUS_NOT_FOUND;
    }
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionGetOption(connection, key, value,
                                                         length, error);
}
...
AdbcStatusCode AdbcConnectionGetOptionBytes(struct AdbcConnection *connection,
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionGetOptionBytes(
      connection, key, value, length, error);
}
...
AdbcStatusCode AdbcConnectionGetOptionInt(struct AdbcConnection *connection,
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionGetOptionInt(connection, key,
                                                            value, error);
}
...
AdbcStatusCode AdbcConnectionGetOptionDouble(struct AdbcConnection *connection,
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionGetOptionDouble(connection, key,
                                                               value, error);
}

AdbcStatusCode AdbcConnectionGetStatistics(
    struct AdbcConnection *connection, const char *catalog,
    const char *db_schema, const char *table_name, char approximate,
    struct ArrowArrayStream *out, struct AdbcError *error) {
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionGetStatistics: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetStatistics(
                  connection, catalog, db_schema, table_name, approximate == 1,
                  out, error),
              out, connection);
}

AdbcStatusCode
AdbcConnectionGetStatisticNames(struct AdbcConnection *connection,
                                struct ArrowArrayStream *out,
                                struct AdbcError *error) {
  if (!connection->private_driver) {
    SetError(
        error,
        "AdbcConnectionGetStatisticNames: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetStatisticNames(
                  connection, out, error),
              out, connection);
}
...
AdbcStatusCode AdbcConnectionGetTableSchema(struct AdbcConnection *connection,
  ...
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionGetTableSchema: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
...
AdbcStatusCode AdbcConnectionGetTableTypes(struct AdbcConnection *connection,
  ...
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionGetTableTypes: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
  WRAP_STREAM(connection->private_driver->ConnectionGetTableTypes(
...
AdbcStatusCode AdbcConnectionReadPartition(struct AdbcConnection *connection,
  ...
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionReadPartition: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
  WRAP_STREAM(
      connection->private_driver->ConnectionReadPartition(
...
AdbcStatusCode AdbcConnectionRollback(struct AdbcConnection *connection,
  ...
  if (!connection->private_driver) {
    SetError(error,
             "AdbcConnectionRollback: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, connection);
...
AdbcStatusCode AdbcConnectionSetOption(struct AdbcConnection *connection,
  ...
  if (!connection->private_data) {
    SetError(error,
             "AdbcConnectionSetOption: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  if (!connection->private_driver) {
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionSetOption(connection, key, value,
                                                         error);
}
...
AdbcStatusCode AdbcConnectionSetOptionBytes(struct AdbcConnection *connection,
  ...
  if (!connection->private_data) {
    SetError(error,
             "AdbcConnectionSetOptionInt: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  if (!connection->private_driver) {
    // Init not yet called, save the option
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionSetOptionBytes(
      connection, key, value, length, error);
}
...
AdbcStatusCode AdbcConnectionSetOptionInt(struct AdbcConnection *connection,
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionSetOptionInt(connection, key,
                                                            value, error);
}
...
AdbcStatusCode AdbcConnectionSetOptionDouble(struct AdbcConnection *connection,
  ...
  if (!connection->private_data) {
    SetError(
        error,
        "AdbcConnectionSetOptionDouble: must call AdbcConnectionNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  if (!connection->private_driver) {
    // Init not yet called, save the option
  ...
  INIT_ERROR(error, connection);
  return connection->private_driver->ConnectionSetOptionDouble(connection, key,
                                                               value, error);
}
...
AdbcStatusCode AdbcStatementBindStream(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementBindStream: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcStatementExecutePartitions(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(
        error,
        "AdbcStatementExecutePartitions: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcStatementExecuteQuery(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementExecuteQuery: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  WRAP_STREAM(statement->private_driver->StatementExecuteQuery(
                  statement, out, rows_affected, error),
              out, statement);
}
...
AdbcStatusCode AdbcStatementExecuteSchema(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementExecuteSchema: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcStatementGetOption(struct AdbcStatement *statement,
  ...
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementGetOption(statement, key, value,
                                                       length, error);
}
...
AdbcStatusCode AdbcStatementGetOptionBytes(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementGetOptionBytes: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementGetOptionBytes(
      statement, key, value, length, error);
}
...
AdbcStatusCode AdbcStatementGetOptionInt(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementGetOptionInt: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementGetOptionInt(statement, key, value,
                                                          error);
}
...
AdbcStatusCode AdbcStatementGetOptionDouble(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementGetOptionDouble: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementGetOptionDouble(statement, key,
                                                             value, error);
}
...
AdbcStatusCode AdbcStatementGetParameterSchema(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(
        error,
        "AdbcStatementGetParameterSchema: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcStatementNew(struct AdbcConnection *connection,
  ...
  INIT_ERROR(error, connection);
  auto status =
      connection->private_driver->StatementNew(connection, statement, error);
...
AdbcStatusCode AdbcStatementSetOptionBytes(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementSetOptionBytes: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementSetOptionBytes(
      statement, key, value, length, error);
}
...
AdbcStatusCode AdbcStatementSetOptionInt(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementSetOptionInt: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementSetOptionInt(statement, key, value,
                                                          error);
}
...
AdbcStatusCode AdbcStatementSetOptionDouble(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementSetOptionDouble: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
  return statement->private_driver->StatementSetOptionDouble(statement, key,
                                                             value, error);
}
...
AdbcStatusCode AdbcStatementSetSqlQuery(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementSetSqlQuery: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcStatementSetSubstraitPlan(struct AdbcStatement *statement,
  ...
  if (!statement->private_driver) {
    SetError(error,
             "AdbcStatementSetSubstraitPlan: must call AdbcStatementNew first");
    return ADBC_STATUS_INVALID_STATE;
  }
  INIT_ERROR(error, statement);
...
AdbcStatusCode AdbcFindLoadDriver(const char *driver_name,
                                  const char *entrypoint, const int version,
                                  const AdbcLoadFlags load_options,
...
AdbcStatusCode AdbcLoadDriver(const char *driver_name, const char *entrypoint,
  ...
  // explicitly enabled by calling AdbcFindLoadDriver directly.
