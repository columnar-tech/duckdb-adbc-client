...
AdbcStatusCode LoadDriverFromRegistry(HKEY root, const std::wstring &driver_name, DriverInfo &info,
...
AdbcStatusCode LoadDriverManifest(const std::filesystem::path &driver_manifest, DriverInfo &info,
                                  struct AdbcError *error) {
	toml::table config;
	try {
...
struct ManagedLibrary {
	...
	AdbcStatusCode GetDriverInfo(const std::string_view driver_name, const AdbcLoadFlags load_options,
	...
	AdbcStatusCode SearchPathsForDriver(const std::filesystem::path &driver_path, SearchPaths &search_paths,
	...
	AdbcStatusCode FindDriver(const std::filesystem::path &driver_path, const AdbcLoadFlags load_options,
	                          const std::vector<std::filesystem::path> &additional_search_paths, DriverInfo &info,
	                          struct AdbcError *error) {
...
AdbcStatusCode AdbcDatabaseInit(struct AdbcDatabase *database, struct AdbcError *error) {
	...
	if (args->init_func) {
		...
		status =
		    AdbcFindLoadDriver(args->driver.c_str(), args->entrypoint.c_str(), ADBC_VERSION_1_1_0, args->load_flags,
		                       args->additional_search_path_list.data(), database->private_driver, error);
	} else {
		status = AdbcFindLoadDriver(args->driver.c_str(), nullptr, ADBC_VERSION_1_1_0, args->load_flags,
		                            args->additional_search_path_list.data(), database->private_driver, error);
	}
	...
	for (const auto &option : options) {
		status = database->private_driver->DatabaseSetOption(database, option.first.c_str(), option.second.c_str(),
		                                                     &driver_error.error);
		if (status != ADBC_STATUS_OK)
			break;
	}
	...
	for (const auto &option : int_options) {
		status = database->private_driver->DatabaseSetOptionInt(database, option.first.c_str(), option.second,
		                                                        &driver_error.error);
		if (status != ADBC_STATUS_OK)
			break;
	}
	for (const auto &option : double_options) {
		status = database->private_driver->DatabaseSetOptionDouble(database, option.first.c_str(), option.second,
		                                                           &driver_error.error);
		if (status != ADBC_STATUS_OK)
			break;
	}
...
AdbcStatusCode AdbcLoadDriver(const char *driver_name, const char *entrypoint, int version, void *raw_driver,
	...
	return AdbcFindLoadDriver(driver_name, entrypoint, version, ADBC_LOAD_FLAG_ALLOW_RELATIVE_PATHS, nullptr,
	                          raw_driver, error);
}
