...
struct FilesystemProfile {
    ...
    void PopulateConnectionProfile(struct AdbcConnectionProfile *out) {
        ...
        out->GetDriverName = [](AdbcConnectionProfile *profile,
                                const char **out,
        ...
        out->GetOptions = [](AdbcConnectionProfile *profile,
                             const char ***keys,
                             const char ***values,
                             size_t *num_options,
                             struct AdbcError *error) -> AdbcStatusCode {
        ...
        out->GetIntOptions = [](AdbcConnectionProfile *profile,
                                const char ***keys,
        ...
        out->GetDoubleOptions = [](AdbcConnectionProfile *profile,
                                   const char ***keys,
                                   const double **values,
                                   size_t *num_options,
                                   struct AdbcError *error) -> AdbcStatusCode {
    ...
    bool VisitTable(const std::string &prefix, toml::table &table) {
        for (const auto &[key, value] : table) {
            if (auto *str = value.as_string()) {
                ...
                std::string message = "Unsupported value type for key '" + std::string(key.str()) + "' in profile '" +
                                      profile_path.string() + "'";
...
AdbcStatusCode ProcessProfileValue(std::string_view key,
    ...
    for (auto itr = begin; itr != end; ++itr) {
        ...
        if (content.rfind("env_var(", 0) != 0) {
            std::string message =
                "In profile: unsupported interpolation type in key `" + std::string(key) + "`: `" + content + "`";
        ...
        if (content[content.size() - 1] != ')') {
            std::string message =
                "In profile: malformed env_var() in key `" + std::string(key) + "`: missing closing parenthesis";
            SetError(error, message);
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
        ...
        if (env_var_name.empty()) {
            std::string message =
                "In profile: malformed env_var() in key `" + std::string(key) + "`: missing environment variable name";
            SetError(error, message);
            return ADBC_STATUS_INVALID_ARGUMENT;
        }
        ...
        if (required_size != 0) {
            ...
            DWORD actual_size = GetEnvironmentVariableW(local_env_var.c_str(), wvalue.data(), required_size);
...
AdbcStatusCode LoadProfileFile(const std::filesystem::path &profile_path,
    ...
    if (!config["profile_version"].is_integer()) {
        std::string message = "Profile version is not an integer in profile '" + profile_path.string() + "'";
        SetError(error, std::move(message));
        return ADBC_STATUS_INVALID_ARGUMENT;
    }
    ...
    case 1:
        break;
    default: {
        std::string message = "Profile version '" + std::to_string(version) +
    ...
    if (!options.is_table()) {
        std::string message = "Profile options is not a table in profile '" + profile_path.string() + "'";
        SetError(error, std::move(message));
        return ADBC_STATUS_INVALID_ARGUMENT;
    }
...
static SearchPaths GetProfileSearchPaths(const char *additional_search_path_list) {
    ...
    for (const auto &[_, venv_path] : venv) {
        search_paths.emplace_back(SearchPathSource::kConda, venv_path / "etc" / "adbc" / "profiles");
    }
#else
