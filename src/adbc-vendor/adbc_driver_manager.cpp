...
AdbcStatusCode InternalAdbcParseOptions(TempDatabase *db, struct AdbcError *error) {
    ...
    // parse driver for URI. we do this even if it was set via profile
    {
        ...
        if (auto maybe_driver = InternalAdbcParseDriverUri(owned_driver); maybe_driver.has_value()) {
...
AdbcStatusCode InternalInitializeProfile(TempDatabase *args, const std::string_view profile, struct AdbcError *error) {
    ...
    CHECK_STATUS(guard.profile.GetOptions(&guard.profile, &keys, &values, &num_options, error));
    ...
    CHECK_STATUS(guard.profile.GetIntOptions(&guard.profile, &keys, &int_values, &num_options, error));
    for (size_t i = 0; i < num_options; ++i) {
        // use try_emplace so we only add the option if there isn't
        // already an option with the same name
    ...
    CHECK_STATUS(guard.profile.GetDoubleOptions(&guard.profile, &keys, &double_values, &num_options, error));
    for (size_t i = 0; i < num_options; ++i) {
