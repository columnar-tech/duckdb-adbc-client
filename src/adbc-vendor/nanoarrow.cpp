...
static uint8_t *ArrowBufferAllocatorMallocReallocate(struct ArrowBufferAllocator *allocator,
                                                     uint8_t *ptr,
                                                     int64_t old_size,
                                                     int64_t new_size) {
...
static struct ArrowBufferAllocator ArrowBufferAllocatorMalloc = {&ArrowBufferAllocatorMallocReallocate,
                                                                 &ArrowBufferAllocatorMallocFree,
                                                                 NULL};
...
struct ArrowBufferAllocator ArrowBufferDeallocator(void (*custom_free)(struct ArrowBufferAllocator *allocator,
                                                                       uint8_t *ptr,
                                                                       int64_t size),
                                                   void *private_data) {
...
ArrowErrorCode ArrowDecimalAppendDigitsToBuffer(const struct ArrowDecimal *decimal, struct ArrowBuffer *buffer) {
    NANOARROW_DCHECK(decimal->n_words == 0 || decimal->n_words == 1 || decimal->n_words == 2 || decimal->n_words == 4);
    ...
    int n_chars =
        snprintf((char *)buffer->data + buffer->size_bytes, 21, "%lu", (unsigned long)segments[num_segments - 1]);
    ...
    for (int i = num_segments - 2; i >= 0; i--) {
        int n_chars = snprintf((char *)buffer->data + buffer->size_bytes, 21, "%09lu", (unsigned long)segments[i]);
        buffer->size_bytes += n_chars;
...
ArrowErrorCode ArrowDecimalAppendStringToBuffer(const struct ArrowDecimal *decimal, struct ArrowBuffer *buffer) {
    ...
    if (num_digits <= decimal->scale) {
        ...
        NANOARROW_RETURN_NOT_OK(ArrowBufferResize(buffer, buffer->size_bytes + num_zeros_after_decimal + 2, 0));
...
static int ArrowSchemaInitChildrenIfNeeded(struct ArrowSchema *schema, enum ArrowType type) {
    switch (type) {
    case NANOARROW_TYPE_LIST:
    case NANOARROW_TYPE_LARGE_LIST:
    case NANOARROW_TYPE_FIXED_SIZE_LIST:
    ...
    case NANOARROW_TYPE_MAP:
        NANOARROW_RETURN_NOT_OK(ArrowSchemaAllocateChildren(schema, 1));
        NANOARROW_RETURN_NOT_OK(ArrowSchemaInitFromType(schema->children[0], NANOARROW_TYPE_STRUCT));
        ...
        NANOARROW_RETURN_NOT_OK(ArrowSchemaSetName(schema->children[0]->children[1], "value"));
        break;
    case NANOARROW_TYPE_RUN_END_ENCODED:
...
ArrowErrorCode ArrowSchemaSetTypeDecimal(struct ArrowSchema *schema,
    ...
    case NANOARROW_TYPE_DECIMAL32:
        ...
        n_chars = snprintf(buffer, sizeof(buffer), "d:%d,%d,32", decimal_precision, decimal_scale);
        break;
    case NANOARROW_TYPE_DECIMAL64:
...
ArrowErrorCode ArrowSchemaSetTypeRunEndEncoded(struct ArrowSchema *schema, enum ArrowType run_end_type) {
    ...
    NANOARROW_RETURN_NOT_OK(ArrowSchemaSetFormat(schema, ArrowSchemaFormatTemplate(NANOARROW_TYPE_RUN_END_ENCODED)));
    NANOARROW_RETURN_NOT_OK(ArrowSchemaInitChildrenIfNeeded(schema, NANOARROW_TYPE_RUN_END_ENCODED));
    NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema->children[0], run_end_type));
    NANOARROW_RETURN_NOT_OK(ArrowSchemaSetType(schema->children[1], NANOARROW_TYPE_UNINITIALIZED));
...
static void ArrowSchemaViewSetPrimitive(struct ArrowSchemaView *schema_view, enum ArrowType type) {
    ...
    *format_end_out = format;
    ...
    case '+':
        ...
        case 'w':
            ...
            schema_view->fixed_size = (int32_t)strtol(format + 3, (char **)format_end_out, 10);
            return NANOARROW_OK;
        case 's':
        ...
        case 'u':
            ...
            if (format[3] == ':') {
                ...
                int64_t n_type_ids = _ArrowParseUnionTypeIds(schema_view->union_type_ids, NULL);
        ...
        case 'v':
            ...
            case 'L':
                ...
                *format_end_out = format + 3;
                return NANOARROW_OK;
            default:
                ArrowErrorSet(error, "Expected view format string +vl or +vL but found '%s'", format);
                return EINVAL;
            }
        default:
            ArrowErrorSet(error, "Expected nested type format string but found '%s'", format);
            return EINVAL;
        }
    ...
    case 't':
        ...
        case 'd':
            ...
            case 'm':
                ...
                *format_end_out = format + 3;
                return NANOARROW_OK;
            default:
                ArrowErrorSet(error, "Expected 'D' or 'm' following 'td' but found '%s'", format + 2);
                return EINVAL;
            }
        ...
        case 't':
            ...
            case 'n':
                ...
                *format_end_out = format + 3;
                return NANOARROW_OK;
            default:
                ArrowErrorSet(error, "Expected 's', 'm', 'u', or 'n' following 'tt' but found '%s'", format + 2);
                return EINVAL;
            }
        ...
        case 's':
            ...
            case 'n':
                ...
                break;
            default:
                ArrowErrorSet(error, "Expected 's', 'm', 'u', or 'n' following 'ts' but found '%s'", format + 2);
                return EINVAL;
            }

            if (format[3] != ':') {
                ArrowErrorSet(error, "Expected ':' following '%.3s' but found '%s'", format, format + 3);
                return EINVAL;
            }
        ...
        case 'D':
            ...
            case 'n':
                ...
                *format_end_out = format + 3;
                return NANOARROW_OK;
            default:
                ArrowErrorSet(error, "Expected 's', 'm', u', or 'n' following 'tD' but found '%s'", format + 2);
                return EINVAL;
            }
        ...
        case 'i':
            ...
            case 'D':
                ...
                *format_end_out = format + 3;
                return NANOARROW_OK;
            case 'n':
                ArrowSchemaViewSetPrimitive(schema_view, NANOARROW_TYPE_INTERVAL_MONTH_DAY_NANO);
                *format_end_out = format + 3;
                return NANOARROW_OK;
            default:
                ArrowErrorSet(error, "Expected 'M', 'D', or 'n' following 'ti' but found '%s'", format + 2);
                return EINVAL;
            }

        default:
            ArrowErrorSet(error, "Expected 'd', 't', 's', 'D', or 'i' following 't' but found '%s'", format + 1);
            return EINVAL;
        }
    ...
    case 'v': {
        ...
        case 'z':
            ...
            *format_end_out = format + 2;
            return NANOARROW_OK;
        default:
            ArrowErrorSet(error, "Expected 'u', or 'z' following 'v' but found '%s'", format + 1);
            return EINVAL;
        }
    }
...
static ArrowErrorCode ArrowSchemaViewValidateNChildren(struct ArrowSchemaView *schema_view,
                                                       int64_t n_children,
                                                       struct ArrowError *error) {
    if (n_children != -1 && schema_view->schema->n_children != n_children) {
        ArrowErrorSet(error,
                      "Expected schema with %" PRId64 " children but found %" PRId64 " children",
                      n_children,
                      schema_view->schema->n_children);
        return EINVAL;
    }
    ...
    for (int64_t i = 0; i < schema_view->schema->n_children; i++) {
        ...
        if (child == NULL) {
            ArrowErrorSet(error, "Expected valid schema at schema->children[%" PRId64 "] but found NULL", i);
            return EINVAL;
        } else if (child->release == NULL) {
            ArrowErrorSet(error,
                          "Expected valid schema at schema->children[%" PRId64 "] but found a released schema",
                          i);
            return EINVAL;
        }
    }
...
static ArrowErrorCode ArrowSchemaViewValidateUnion(struct ArrowSchemaView *schema_view, struct ArrowError *error) {
    return ArrowSchemaViewValidateNChildren(schema_view, -1, error);
}

static ArrowErrorCode ArrowSchemaViewValidateMap(struct ArrowSchemaView *schema_view, struct ArrowError *error) {
    ...
    if (schema_view->schema->children[0]->flags & ARROW_FLAG_NULLABLE) {
        ArrowErrorSet(error, "Expected child of map type to be non-nullable but was nullable");
        return EINVAL;
    }
...
static ArrowErrorCode ArrowSchemaViewValidateDictionary(struct ArrowSchemaView *schema_view, struct ArrowError *error) {
    ...
    return ArrowSchemaViewInit(&dictionary_schema_view, schema_view->schema->dictionary, error);
}
...
static ArrowErrorCode ArrowSchemaViewValidate(struct ArrowSchemaView *schema_view,
    ...
    case NANOARROW_TYPE_FIXED_SIZE_BINARY:
        if (schema_view->fixed_size <= 0) {
            ArrowErrorSet(error, "Expected size > 0 for fixed size binary but found size %d", schema_view->fixed_size);
            return EINVAL;
        }
        return ArrowSchemaViewValidateNChildren(schema_view, 0, error);
    ...
    default:
        ArrowErrorSet(error, "Expected a valid enum ArrowType value but found %d", schema_view->type);
        return EINVAL;
    }
...
ArrowErrorCode ArrowSchemaViewInit(struct ArrowSchemaView *schema_view,
    ...
    if (format == NULL) {
        ArrowErrorSet(error, "Error parsing schema->format: Expected a null-terminated string but found NULL");
        return EINVAL;
    }
    ...
    if ((format + format_len) != format_end_out) {
        ArrowErrorSet(error,
                      "Error parsing schema->format '%s': parsed %d/%zu characters",
                      format,
                      (int)(format_end_out - format),
                      format_len);
        return EINVAL;
    }
    ...
    NANOARROW_RETURN_NOT_OK(ArrowSchemaViewValidate(schema_view, schema_view->storage_type, error));

    if (schema_view->storage_type != schema_view->type) {
        NANOARROW_RETURN_NOT_OK(ArrowSchemaViewValidate(schema_view, schema_view->type, error));
    }
    ...
    if (schema->flags & ARROW_FLAG_DICTIONARY_ORDERED && schema_view->type != NANOARROW_TYPE_DICTIONARY) {
        ArrowErrorSet(error, "ARROW_FLAG_DICTIONARY_ORDERED is only relevant for dictionaries");
        return EINVAL;
    }

    if (schema->flags & ARROW_FLAG_MAP_KEYS_SORTED && schema_view->type != NANOARROW_TYPE_MAP) {
    ...
    NANOARROW_RETURN_NOT_OK(
        ArrowMetadataGetValue(schema->metadata, ArrowCharView("ARROW:extension:name"), &schema_view->extension_name));
...
static int64_t ArrowSchemaTypeToStringInternal(struct ArrowSchemaView *schema_view, char *out, int64_t n) {
    ...
    case NANOARROW_TYPE_TIME64:
    case NANOARROW_TYPE_DURATION:
        return snprintf(out, n, "%s('%s')", type_string, ArrowTimeUnitString(schema_view->time_unit));
    case NANOARROW_TYPE_FIXED_SIZE_BINARY:
    case NANOARROW_TYPE_FIXED_SIZE_LIST:
...
static inline void ArrowToStringLogChars(char **out, int64_t n_chars_last, int64_t *n_remaining, int64_t *n_chars) {
...
int64_t ArrowSchemaToString(const struct ArrowSchema *schema, char *out, int64_t n, char recursive) {
    ...
    if (is_extension && is_dictionary) {
        ...
        n_chars_last =
            snprintf(out, n, "%.*s{", (int)schema_view.extension_name.size_bytes, schema_view.extension_name.data);
    } else if (is_dictionary) {
        n_chars_last = snprintf(out, n, "dictionary(%s)<", ArrowTypeString(schema_view.storage_type));
    }
...
static ArrowErrorCode ArrowMetadataGetValueInternal(const char *metadata,
    ...
    while (ArrowMetadataReaderRead(&reader, &existing_key, &existing_value) == NANOARROW_OK) {
...
ArrowErrorCode ArrowMetadataGetValue(const char *metadata,
                                     struct ArrowStringView key,
                                     struct ArrowStringView *value_out) {
    if (value_out == NULL) {
        return EINVAL;
...
static ArrowErrorCode ArrowMetadataBuilderAppendInternal(struct ArrowBuffer *buffer,
    ...
    NANOARROW_RETURN_NOT_OK(ArrowBufferReserve(buffer, sizeof(int32_t) + key_size + sizeof(int32_t) + value_size));
...
static ArrowErrorCode ArrowMetadataBuilderSetInternal(struct ArrowBuffer *buffer,
    ...
    NANOARROW_RETURN_NOT_OK(ArrowMetadataGetValueInternal((const char *)buffer->data, key, &current_value));
    ...
    while (reader.remaining_keys > 0) {
        ...
        if (key->size_bytes == existing_key.size_bytes &&
            strncmp((const char *)key->data, (const char *)existing_key.data, existing_key.size_bytes) == 0) {
            result = ArrowMetadataBuilderAppendInternal(&new_buffer, key, value);
            value = NULL;
        } else {
            result = ArrowMetadataBuilderAppendInternal(&new_buffer, &existing_key, &existing_value);
        }
...
static ArrowErrorCode ArrowArraySetStorageType(struct ArrowArray *array, enum ArrowType storage_type) {
    switch (storage_type) {
    case NANOARROW_TYPE_UNINITIALIZED:
    case NANOARROW_TYPE_NA:
    case NANOARROW_TYPE_RUN_END_ENCODED:
...
ArrowErrorCode ArrowArrayInitFromType(struct ArrowArray *array, enum ArrowType storage_type) {
    ...
    private_data->buffer_data = (const void **)ArrowMalloc(sizeof(void *) * NANOARROW_MAX_FIXED_BUFFERS);
...
ArrowErrorCode ArrowArrayInitFromArrayView(struct ArrowArray *array,
                                           ...
                                           struct ArrowError *error) {
    NANOARROW_RETURN_NOT_OK_WITH_ERROR(ArrowArrayInitFromType(array, array_view->storage_type), error);
    int result;
    ...
    if (array_view->n_children > 0) {
        ...
        for (int64_t i = 0; i < array_view->n_children; i++) {
            result = ArrowArrayInitFromArrayView(array->children[i], array_view->children[i], error);
            if (result != NANOARROW_OK) {
                ArrowArrayRelease(array);
                return result;
            }
    ...
    if (array_view->dictionary != NULL) {
        ...
        result = ArrowArrayInitFromArrayView(array->dictionary, array_view->dictionary, error);
        if (result != NANOARROW_OK) {
            ArrowArrayRelease(array);
            return result;
        }
...
ArrowErrorCode ArrowArrayAllocateChildren(struct ArrowArray *array, int64_t n_children) {
    ...
    array->children = (struct ArrowArray **)ArrowMalloc(n_children * sizeof(struct ArrowArray *));
    if (array->children == NULL) {
        return ENOMEM;
    }
...
static ArrowErrorCode ArrowArrayViewInitFromArray(struct ArrowArrayView *array_view,
    ...
    if (!ArrowArrayIsInternal(array)) {
        ArrowErrorSet(error, "Can't initialize internal ArrowArrayView from external ArrowArray");
        return EINVAL;
    }
    ...
    for (int64_t i = 0; i < array->n_children; i++) {
        result = ArrowArrayViewInitFromArray(array_view->children[i], array->children[i], error);
        if (result != NANOARROW_OK) {
            ArrowArrayViewReset(array_view);
            return result;
        }
    ...
    if (array->dictionary != NULL) {
        ...
        result = ArrowArrayViewInitFromArray(array_view->dictionary, array->dictionary, error);
        if (result != NANOARROW_OK) {
            ArrowArrayViewReset(array_view);
            return result;
        }
...
static ArrowErrorCode ArrowArrayReserveInternal(struct ArrowArray *array, struct ArrowArrayView *array_view) {
    ...
    for (int64_t i = 0; i < NANOARROW_MAX_FIXED_BUFFERS; i++) {
        ...
        int64_t additional_size_bytes = array_view->buffer_views[i].size_bytes - ArrowArrayBuffer(array, i)->size_bytes;

        if (additional_size_bytes > 0) {
            NANOARROW_RETURN_NOT_OK(ArrowBufferReserve(ArrowArrayBuffer(array, i), additional_size_bytes));
        }
    }

    // Recursively reserve children
    for (int64_t i = 0; i < array->n_children; i++) {
        NANOARROW_RETURN_NOT_OK(ArrowArrayReserveInternal(array->children[i], array_view->children[i]));
    }
...
static ArrowErrorCode ArrowArrayFlushInternalPointers(struct ArrowArray *array) {
    ...
    if (array->n_buffers > NANOARROW_MAX_FIXED_BUFFERS) {
        ...
        if (sizes_buffer->data == NULL && sizes_buffer->size_bytes == 0) {
            NANOARROW_RETURN_NOT_OK(ArrowBufferReserve(sizes_buffer, private_data->n_variadic_buffers));
            for (int64_t i = 0; i < private_data->n_variadic_buffers; i++) {
                struct ArrowBuffer *variadic_buffer = ArrowArrayBuffer(array, i + NANOARROW_BINARY_VIEW_FIXED_BUFFERS);
                NANOARROW_RETURN_NOT_OK(ArrowBufferAppendInt64(sizes_buffer, variadic_buffer->size_bytes));
            }
        }
    }
...
ArrowErrorCode ArrowArrayFinishBuilding(struct ArrowArray *array,
    ...
    struct ArrowArrayView array_view;
    NANOARROW_RETURN_NOT_OK_WITH_ERROR(ArrowArrayViewInitFromArray(&array_view, array, error), error);
...
ArrowErrorCode ArrowArrayViewAllocateChildren(struct ArrowArrayView *array_view, int64_t n_children) {
    ...
    array_view->children = (struct ArrowArrayView **)ArrowMalloc(n_children * sizeof(struct ArrowArrayView *));
    if (array_view->children == NULL) {
        return ENOMEM;
    }
    ...
    for (int64_t i = 0; i < n_children; i++) {
        array_view->children[i] = (struct ArrowArrayView *)ArrowMalloc(sizeof(struct ArrowArrayView));
...
ArrowErrorCode ArrowArrayViewAllocateDictionary(struct ArrowArrayView *array_view) {
    ...
    array_view->dictionary = (struct ArrowArrayView *)ArrowMalloc(sizeof(struct ArrowArrayView));
    if (array_view->dictionary == NULL) {
        return ENOMEM;
    }
...
ArrowErrorCode ArrowArrayViewInitFromSchema(struct ArrowArrayView *array_view,
    ...
    for (int64_t i = 0; i < schema->n_children; i++) {
        result = ArrowArrayViewInitFromSchema(array_view->children[i], schema->children[i], error);
        if (result != NANOARROW_OK) {
            ArrowArrayViewReset(array_view);
            return result;
        }
    ...
    if (schema->dictionary != NULL) {
        ...
        result = ArrowArrayViewInitFromSchema(array_view->dictionary, schema->dictionary, error);
        if (result != NANOARROW_OK) {
            ArrowArrayViewReset(array_view);
            return result;
        }
    ...
    if (array_view->storage_type == NANOARROW_TYPE_SPARSE_UNION ||
        ...
        int32_t n_type_ids = _ArrowParseUnionTypeIds(schema_view.union_type_ids, array_view->union_type_id_map + 128);
...
void ArrowArrayViewSetLength(struct ArrowArrayView *array_view, int64_t length) {
    for (int i = 0; i < NANOARROW_MAX_FIXED_BUFFERS; i++) {
        ...
        case NANOARROW_BUFFER_TYPE_DATA_OFFSET:
            ...
            array_view->buffer_views[i].size_bytes = (length != 0) * element_size_bytes * (length + 1);
            continue;
        case NANOARROW_BUFFER_TYPE_DATA:
            array_view->buffer_views[i].size_bytes =
                _ArrowRoundUpToMultipleOf8(array_view->layout.element_size_bits[i] * length) / 8;
            continue;
    ...
    case NANOARROW_TYPE_FIXED_SIZE_LIST:
        if (array_view->n_children >= 1) {
            ArrowArrayViewSetLength(array_view->children[0], length * array_view->layout.child_size_elements);
        }
    default:
        break;
    }
...
static int ArrowArrayViewSetArrayInternal(struct ArrowArrayView *array_view,
    ...
    const int nfixed_buf =
        array_view->storage_type == NANOARROW_TYPE_STRING_VIEW || array_view->storage_type == NANOARROW_TYPE_BINARY_VIEW
    ...
    if (buffers_required != array->n_buffers) {
        ArrowErrorSet(error,
                      "Expected array with %" PRId64 " buffer(s) but found %" PRId64 " buffer(s)",
                      buffers_required,
                      array->n_buffers);
        return EINVAL;
    }
    ...
    if (array_view->n_children != array->n_children) {
        ArrowErrorSet(error,
                      "Expected %" PRId64 " children but found %" PRId64 " children",
    ...
    for (int64_t i = 0; i < array_view->n_children; i++) {
        NANOARROW_RETURN_NOT_OK(ArrowArrayViewSetArrayInternal(array_view->children[i], array->children[i], error));
    }
    ...
    if (array->dictionary != NULL) {
        NANOARROW_RETURN_NOT_OK(ArrowArrayViewSetArrayInternal(array_view->dictionary, array->dictionary, error));
    }

    return NANOARROW_OK;
}

static int ArrowArrayViewValidateMinimal(struct ArrowArrayView *array_view, struct ArrowError *error) {
    if (array_view->length < 0) {
        ArrowErrorSet(error, "Expected length >= 0 but found length %" PRId64, array_view->length);
        return EINVAL;
    }

    if (array_view->offset < 0) {
        ArrowErrorSet(error, "Expected offset >= 0 but found offset %" PRId64, array_view->offset);
        return EINVAL;
    }
    ...
    for (int i = 0; i < 2; i++) {
        ...
        case NANOARROW_BUFFER_TYPE_DATA_OFFSET:
            // Probably don't want/need to rely on the producer to have allocated an
            // offsets buffer of length 1 for a zero-size array
            min_buffer_size_bytes = (offset_plus_length != 0) * element_size_bytes * (offset_plus_length + 1);
            break;
        case NANOARROW_BUFFER_TYPE_VIEW_OFFSET:
            min_buffer_size_bytes = (offset_plus_length != 0) * element_size_bytes * offset_plus_length;
            break;
        case NANOARROW_BUFFER_TYPE_DATA:
            min_buffer_size_bytes =
                _ArrowRoundUpToMultipleOf8(array_view->layout.element_size_bits[i] * offset_plus_length) / 8;
            break;
        case NANOARROW_BUFFER_TYPE_TYPE_ID:
        case NANOARROW_BUFFER_TYPE_UNION_OFFSET:
            min_buffer_size_bytes = element_size_bytes * offset_plus_length;
        ...
        if (array_view->buffer_views[i].size_bytes == -1) {
            ...
            ArrowErrorSet(error,
                          "Expected %s array buffer %d to have size >= %" PRId64 " bytes but found "
    ...
    case NANOARROW_TYPE_RUN_END_ENCODED:
        if (array_view->n_children != 2) {
            ArrowErrorSet(error,
                          "Expected 2 children for %s array but found %" PRId64 " child arrays",
                          ArrowTypeString(array_view->storage_type),
                          array_view->n_children);
            return EINVAL;
        }
    ...
    case NANOARROW_TYPE_STRUCT:
        ...
        for (int64_t i = 0; i < array_view->n_children; i++) {
            if (array_view->children[i]->length < child_min_length) {
                ArrowErrorSet(error,
                              "Expected struct child %" PRId64 " to have length >= %" PRId64 " but found child with "
                              "length %" PRId64,
                              i + 1,
                              child_min_length,
    ...
    case NANOARROW_TYPE_FIXED_SIZE_LIST:
        child_min_length = (array_view->offset + array_view->length) * array_view->layout.child_size_elements;
        if (array_view->children[0]->length < child_min_length) {
            ArrowErrorSet(error,
                          "Expected child of fixed_size_list array to have length >= %" PRId64 " but "
    ...
    case NANOARROW_TYPE_RUN_END_ENCODED: {
        ...
        if (run_ends_view->length > values_view->length) {
            ArrowErrorSet(error,
                          "Length of run_ends is greater than the length of values: %" PRId64 " > %" PRId64,
        ...
        if (run_ends_view->length == 0 && values_view->length != 0) {
            ArrowErrorSet(error,
                          "Run-end encoded array has zero length %" PRId64 ", but values array has "
                          "non-zero length",
                          values_view->length);
            return EINVAL;
        }
        ...
        if (run_ends_view->null_count != 0) {
            ArrowErrorSet(error, "Null count must be 0 for run ends array, but is %" PRId64, run_ends_view->null_count);
            return EINVAL;
        }
        break;
    }
    ...
    // Recurse for children
    for (int64_t i = 0; i < array_view->n_children; i++) {
        NANOARROW_RETURN_NOT_OK(ArrowArrayViewValidateMinimal(array_view->children[i], error));
    }
...
static int ArrowArrayViewValidateDefault(struct ArrowArrayView *array_view, struct ArrowError *error) {
    ...
    case NANOARROW_TYPE_BINARY:
        if (array_view->buffer_views[1].size_bytes != 0) {
            ...
            if (first_offset < 0) {
                ArrowErrorSet(error, "Expected first offset >= 0 but found %" PRId64, first_offset);
                return EINVAL;
            }

            last_offset = array_view->buffer_views[1].data.as_int32[offset_plus_length];
            if (last_offset < 0) {
                ArrowErrorSet(error, "Expected last offset >= 0 but found %" PRId64, last_offset);
                return EINVAL;
            }
            ...
            if (array_view->buffer_views[2].size_bytes == -1) {
                ...
                ArrowErrorSet(error,
                              "Expected %s array buffer 2 to have size >= %" PRId64 " bytes but found "
                              "buffer with %" PRId64 " bytes",
                              ArrowTypeString(array_view->storage_type),
                              last_offset,
    ...
    case NANOARROW_TYPE_MAP:
        if (array_view->buffer_views[1].size_bytes != 0) {
            ...
            if (array_view->children[0]->length < last_offset) {
                ArrowErrorSet(error,
                              "Expected child of %s array to have length >= %" PRId64 " but found array with "
                              "length %" PRId64,
                              ArrowTypeString(array_view->storage_type),
                              last_offset,
                              array_view->children[0]->length);
    ...
    case NANOARROW_TYPE_LARGE_LIST:
        if (array_view->buffer_views[1].size_bytes != 0) {
            ...
            if (array_view->children[0]->length < last_offset) {
                ArrowErrorSet(error,
                              "Expected child of %s array to have length >= %" PRId64 " but found array "
                              "with length %" PRId64,
                              ArrowTypeString(array_view->storage_type),
                              last_offset,
                              array_view->children[0]->length);
    ...
    case NANOARROW_TYPE_RUN_END_ENCODED: {
        ...
        int64_t last_run_end = ArrowArrayViewGetIntUnsafe(run_ends_view, run_ends_view->length - 1);
        if (last_run_end < offset_plus_length) {
            ArrowErrorSet(error,
                          "Last run end is %" PRId64 " but it should be >= (%" PRId64 " + %" PRId64 ")",
                          last_run_end,
                          array_view->offset,
                          array_view->length);
            return EINVAL;
    ...
    // Recurse for children
    for (int64_t i = 0; i < array_view->n_children; i++) {
        NANOARROW_RETURN_NOT_OK(ArrowArrayViewValidateDefault(array_view->children[i], error));
    }
...
static int ArrowAssertIncreasingInt32(struct ArrowBufferView view, struct ArrowError *error) {
...
static int ArrowAssertIncreasingInt64(struct ArrowBufferView view, struct ArrowError *error) {
...
static int ArrowAssertRangeInt8(struct ArrowBufferView view,
                                int8_t min_value,
                                int8_t max_value,
                                struct ArrowError *error) {
...
static int ArrowAssertInt8In(struct ArrowBufferView view,
                             const int8_t *values,
                             int64_t n_values,
                             struct ArrowError *error) {
    for (int64_t i = 0; i < view.size_bytes; i++) {
        ...
        if (!item_found) {
            ArrowErrorSet(error, "[%" PRId64 "] Unexpected buffer value %" PRId8, i, view.data.as_int8[i]);
            return EINVAL;
        }
    }
...
static int ArrowArrayViewValidateFull(struct ArrowArrayView *array_view, struct ArrowError *error) {
    for (int i = 0; i < NANOARROW_MAX_FIXED_BUFFERS; i++) {
        switch (array_view->layout.buffer_type[i]) {
        ...
        case NANOARROW_BUFFER_TYPE_DATA_OFFSET:
            ...
            if (array_view->layout.element_size_bits[i] == 32) {
                ...
                sliced_offsets.data.as_int32 = array_view->buffer_views[i].data.as_int32 + array_view->offset;
                ...
                struct ArrowBufferView sliced_offsets;
                sliced_offsets.data.as_int64 = array_view->buffer_views[i].data.as_int64 + array_view->offset;
    ...
    if (array_view->storage_type == NANOARROW_TYPE_DENSE_UNION ||
        ...
        if (array_view->length > 0) {
            sliced_type_ids.data.as_int8 = array_view->buffer_views[0].data.as_int8 + array_view->offset;
        } else {
        ...
        if (array_view->union_type_id_map == NULL) {
            ...
            ArrowErrorSet(error, "Insufficient information provided for validation of union array");
            return EINVAL;
        } else if (_ArrowParsedUnionTypeIdsWillEqualChildIndices(array_view->union_type_id_map,
                                                                 array_view->n_children,
                                                                 array_view->n_children)) {
            NANOARROW_RETURN_NOT_OK(
                ArrowAssertRangeInt8(sliced_type_ids, 0, (int8_t)(array_view->n_children - 1), error));
        } else {
            NANOARROW_RETURN_NOT_OK(
                ArrowAssertInt8In(sliced_type_ids, array_view->union_type_id_map + 128, array_view->n_children, error));
        }
    }

    if (array_view->storage_type == NANOARROW_TYPE_DENSE_UNION && array_view->union_type_id_map != NULL) {
        ...
        for (int64_t i = 0; i < array_view->length; i++) {
            ...
            if (offset < 0 || offset > child_length) {
                ArrowErrorSet(error,
                              "[%" PRId64 "] Expected union offset for child id %" PRId8 " to be between 0 and %" PRId64
                              " but "
            ...
            for (int64_t i = 1; i < run_ends_view->length; i++) {
                ...
                if (run_end <= last_run_end) {
                    ArrowErrorSet(error,
                                  ...
                                  "but run_ends[%" PRId64 " is %" PRId64 " and run_ends[%" PRId64 "] is %" PRId64,
                                  i,
                                  run_end,
                                  i - 1,
                                  last_run_end);
    ...
    if (array_view->storage_type == NANOARROW_TYPE_LIST_VIEW ||
        ...
        for (int64_t i = array_view->offset; i < array_view->length + array_view->offset; i++) {
            int64_t offset, size;
            if (array_view->storage_type == NANOARROW_TYPE_LIST_VIEW) {
            ...
            if (offset < 0) {
                ArrowErrorSet(error, "Invalid negative offset %" PRId64 " at index %" PRId64, offset, i);
                return EINVAL;
            }

            if (size < 0) {
                ArrowErrorSet(error, "Invalid negative size %" PRId64 " at index %" PRId64, size, i);
                return EINVAL;
            }
...
NANOARROW_CHECK_PRINTF_ATTRIBUTE static void ArrowComparePrependPath(struct ArrowError *out, const char *fmt, ...) {
    if (out == NULL) {
        return;
    }
...
static void ArrowArrayViewCompareBuffer(const struct ArrowArrayView *actual,
    ...
    SET_NOT_EQUAL_AND_RETURN_IF(actual->buffer_views[i].size_bytes != expected->buffer_views[i].size_bytes, state);

    int64_t buffer_size = actual->buffer_views[i].size_bytes;
    if (buffer_size > 0) {
        SET_NOT_EQUAL_AND_RETURN_IF(
            memcmp(actual->buffer_views[i].data.data, expected->buffer_views[i].data.data, buffer_size) != 0,
            state);
    }
}
...
static void ArrowArrayViewCompareIdentical(const struct ArrowArrayView *actual,
    ...
    SET_NOT_EQUAL_AND_RETURN_IF(actual->dictionary == NULL && expected->dictionary != NULL, state);
    SET_NOT_EQUAL_AND_RETURN_IF(actual->dictionary != NULL && expected->dictionary == NULL, state);
...
static int ArrowBasicArrayStreamGetSchema(struct ArrowArrayStream *array_stream, struct ArrowSchema *schema) {
    if (array_stream == NULL || array_stream->release == NULL) {
        return EINVAL;
    }

    struct BasicArrayStreamPrivate *private_data = (struct BasicArrayStreamPrivate *)array_stream->private_data;
    return ArrowSchemaDeepCopy(&private_data->schema, schema);
}

static int ArrowBasicArrayStreamGetNext(struct ArrowArrayStream *array_stream, struct ArrowArray *array) {
    if (array_stream == NULL || array_stream->release == NULL) {
        return EINVAL;
    }
...
static const char *ArrowBasicArrayStreamGetLastError(struct ArrowArrayStream *array_stream) {
...
ArrowErrorCode ArrowBasicArrayStreamInit(struct ArrowArrayStream *array_stream,
    ...
    struct BasicArrayStreamPrivate *private_data =
        (struct BasicArrayStreamPrivate *)ArrowMalloc(sizeof(struct BasicArrayStreamPrivate));
    if (private_data == NULL) {
        return ENOMEM;
    }
    ...
    if (n_arrays > 0) {
        private_data->arrays = (struct ArrowArray *)ArrowMalloc(n_arrays * sizeof(struct ArrowArray));
        if (private_data->arrays == NULL) {
...
void ArrowBasicArrayStreamSetArray(struct ArrowArrayStream *array_stream, int64_t i, struct ArrowArray *array) {
    struct BasicArrayStreamPrivate *private_data = (struct BasicArrayStreamPrivate *)array_stream->private_data;
    ArrowArrayMove(array, &private_data->arrays[i]);
}

ArrowErrorCode ArrowBasicArrayStreamValidate(const struct ArrowArrayStream *array_stream, struct ArrowError *error) {
    struct BasicArrayStreamPrivate *private_data = (struct BasicArrayStreamPrivate *)array_stream->private_data;

    struct ArrowArrayView array_view;
    NANOARROW_RETURN_NOT_OK(ArrowArrayViewInitFromSchema(&array_view, &private_data->schema, error));
