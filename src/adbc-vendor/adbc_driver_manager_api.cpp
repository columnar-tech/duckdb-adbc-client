...
static int ErrorArrayStreamGetNext(struct ArrowArrayStream *stream, struct ArrowArray *array) {
...
static int ErrorArrayStreamGetSchema(struct ArrowArrayStream *stream, struct ArrowSchema *schema) {
    if (stream->release != ErrorArrayStreamRelease || !stream->private_data) return EINVAL;
    auto *private_data = reinterpret_cast<struct ErrorArrayStream *>(stream->private_data);
    return private_data->stream.get_schema(&private_data->stream, schema);
}

static void ErrorArrayStreamInit(struct ArrowArrayStream *out, struct AdbcDriver *private_driver) {
    if (!out || !out->release ||
...
AdbcStatusCode AdbcDatabaseInit(struct AdbcDatabase *database, struct AdbcError *error) {
    ...
    if (args->init_func) {
        ...
        const char *additional_paths = args->additional_manifest_search_path_list.empty()
                                           ? nullptr
...
AdbcStatusCode AdbcConnectionGetStatistics(struct AdbcConnection *connection,
    ...
    WRAP_STREAM(connection->private_driver
                    ->ConnectionGetStatistics(connection, catalog, db_schema, table_name, approximate, out, error),
                out,
                connection);
}
...
AdbcStatusCode AdbcConnectionGetTableTypes(struct AdbcConnection *connection,
    ...
    INIT_ERROR(error, connection);
    WRAP_STREAM(connection->private_driver->ConnectionGetTableTypes(connection, out, error), out, connection);
}
