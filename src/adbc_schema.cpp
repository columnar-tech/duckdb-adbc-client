...
AdbcSchemaEntry::LookupEntry(CatalogTransaction transaction,
  ...
  string sql = StringUtil::Format(
      "SELECT * FROM read_adbc('%s', 'SELECT * FROM \"%s\".\"%s\"')",
  ...
  owned_views[name] = std::move(view_entry);
  return ptr; 
}
void AdbcSchemaEntry::Scan(
  ...
  auto &adbc_catalog = catalog.Cast<AdbcCatalog>();
  for (const auto &table : GetTableNames(adbc_catalog.GetUri())) {
    auto entry = GetEntry(GetCatalogTransaction(context),
                          CatalogType::VIEW_ENTRY, table);
    if (entry) {
      callback(*entry);
    }
  }
